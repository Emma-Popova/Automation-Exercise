...
test.describe("Automation Exercise Authentication", () => {
  ...
  test("Test Case 1: Register user", async ({ page }) => {
    ...
    //await expect(page).toHaveURL(/\/login$/); По добър
    ...
    const email = "didi.playwright@example.com"
    await page.locator('[data-qa="signup-email"]').fill(email);

    await page.getByRole("button", { name: "Signup" }).click();

    await expect(page).toHaveURL("https://automationexercise.com/signup");
    //await expect(page).toHaveURL(/\/signup$/); По добър
    await expect(page.getByText("Enter Account Information")).toBeVisible();
    //await expect(page.locator(".login-form")).toBeVisible(); Излишно е

    await page.locator("#id_gender2").check();
    await page.locator('[data-qa="password"]').fill("Didi123");
